...
avg_raster <- list.files(result_raster, pattern = "avg.asc$", full.names = T) %>%
  lapply(raster)
...
extract_by_polygon <- function(r, shape, out_dir){
  ...
  avg <- intersect_cuenca_df %>%
    group_by(NameFinal) %>%
    summarize_(avg = paste0('mean(', var_raster, ')')) %>%
    ungroup() %>%
    rename_(.dots = setNames("avg", var_raster))
  
  write_csv(avg, paste0(out_dir, var_raster, ".csv"))
  return(avg)
}
