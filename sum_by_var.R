...
sum_by_var <- function(var, all_dirs, final_name, out_dir){
...
sum_by_var <- foreach(i = 1:length_run, .packages = c('raster', 'tidyverse', 'stringr', 'magrittr')) %dopar% {
  
  
  sum_by_var(vars[i], dirs_raster, final_name, out_dir)
